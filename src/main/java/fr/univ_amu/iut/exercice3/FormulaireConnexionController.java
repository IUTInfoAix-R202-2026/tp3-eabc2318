...
public class FormulaireConnexionController {
  ...
  private void initialize() {
    ...
    // 3. Le bouton OK est désactivé tant que le mot de passe n'est pas valide (trop court, sans
    // majuscule, ou sans chiffre)
    BooleanBinding motDePasseInvalide =
        new BooleanBinding() {
          {
            // Déclaration de la dépendance : on recalcule à chaque modification du texte
          ...
          protected boolean computeValue() {
            ...
            // Retourne true (donc bouton désactivé) si l'une des 3 conditions d'invalidité est
            // remplie
            return mdp.length() < 8
  ...
  private void valider() {
    ...
    // Génère une chaîne de "*" de la même longueur que le mot de passe
    String masque = "*".repeat(mdp.length());
  ...
  private void annuler() {
    ...
    // Pas besoin de reset l'état des boutons ou l'éditabilité :
    // les Bindings configurés dans initialize() s'occupent de tout automatiquement !
  }
}
